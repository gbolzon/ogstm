...
subroutine OrtMatrix(CurrentMatrix, ColumnSize, RowSize, nRows)
    ...
    double precision, dimension (ColumnSize, :), intent(inout) :: CurrentMatrix
    integer :: indexi, indexj
    
    do indexi=1, nRows
        call NormalNumber(CurrentMatrix(:,RowSize+indexi),ColumnSize)
        CurrentMatrix(:,RowSize+indexi)=norm2(CurrentMatrix(:,RowSize+indexi))
        do indexj=1, RowSize+indexi-1
            CurrentMatrix(:,RowSize+indexi)=CurrentMatrix(:,RowSize+indexi)-dot_product(CurrentMatrix(:,RowSize+indexi),CurrentMatrix(:,indexj))*CurrentMatrix(:,indexj)
            CurrentMatrix(:,RowSize+indexi)=norm2(CurrentMatrix(:,RowSize+indexi))
        end do
    end do
end subroutine
